...
```{r}
x <- 1:10
stuff <- tibble(x = 1:5, y = letters[1:5], z = rnorm(5))
...
```{r}
wm_tidy_cleaned <- wm_tidy %>%
  rename(didProbeChange = cond) %>%
  select(-groupStatus) %>%
  select(subjNum, resp, RT, everything()) %>%
  mutate(resp = recode(resp,
         ...
         trialNumAcross = 1:n()) %>%
  group_by(subjNum) %>%
  mutate(RTzWithin = scale(RT),
         trialNumWithin = 1:n())

wm_lms <- wm_tidy_cleaned %>%
  group_by(subjNum) %>%
  nest(.key = "trialwise") %>%
  mutate(model = map(trialwise, ~glm(resp ~ didProbeChange * numDistr,
                                     family = binomial(link = "probit"),
                                     data = .)),
         coefs = map(model, ~broom::tidy(.))) %>%
  unnest(coefs)

wm_summary <- wm_tidy_cleaned %>%
  group_by(subjNum, numDistr) %>%
  summarize(RTmean = mean(RT, na.rm = TRUE),
            RTmedian = median(RT, na.rm = TRUE))

stuff_summary <- tibble(x = rnorm(10),
                        y = rnorm(10),
                        z = rnorm(10),
                        a = letters[1:10]) %>%
  summarize_if(is.numeric, funs(mean(., na.rm = TRUE), median(., na.rm = TRUE))) %>%
  rename_at(vars(ends_with("mean")), funs(str_replace(., "mean", "average")))
  # summarize_at(c("x", "y"), funs(mean(., na.rm = TRUE),
                               #  median(., na.rm = TRUE)))
```
